...
class LitModel(LightningModule):
    ...
    def __init__(
        ...
        # Select the appropriate model class based on model_type
        if self.model_type == "zoo":
            self.model_class = TheZooArchitecture
        elif self.model_type == "transformer":
            self.model_class = SportsTransformer
        else:
            raise ValueError(f"Invalid model_type: {model_type}. Must be 'transformer' or 'zoo'.")

        # 10 features: x, y, s, a, vx, vy, ox, oy, ball_land_x, ball_land_y
        self.feature_len = 10
...
class LitModel_PA(LightningModule):
    """
    Lightning module for training and evaluating play action prediction models (binary classification).
    """
    ...
    def __init__(
        ...
        """
        Initialize the LitModel_PA for play action prediction.
        ...
        # Modify the decoder to output 1 value instead of 2 (for binary classification)
        if self.model_type == "transformer":
            base_model.decoder = nn.Sequential(
                nn.Linear(model_dim, model_dim),
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(model_dim, model_dim // 4),
                nn.ReLU(),
                nn.LayerNorm(model_dim // 4),
                nn.Linear(model_dim // 4, 1),  # Output 1 value for binary classification
            )
        elif self.model_type == "zoo":
            # Update Zoo model output layer for binary classification
            base_model.output_layer = nn.Sequential(
                *(
                    [
                        nn.Sequential(
                            nn.Linear(model_dim, model_dim),
    ...
    def training_step(self, batch: tuple[Tensor, Tensor], batch_idx: int) -> Tensor:
        ...
        Args:
            batch (tuple[Tensor, Tensor]): Batch of input features and target labels.
            batch_idx (int): Index of the current batch.
        ...
        """
        x, y = batch
        y_hat = self.model(x).squeeze(-1)  # Squeeze to match target shape
    ...
    def predict_step(self, batch: tuple[Tensor, Tensor], batch_idx: int, dataloader_idx: int = 0) -> Tensor:
        ...
        Returns:
            Tensor: Predicted probabilities (after sigmoid).
        """
        x, y = batch
        y_hat = self.model(x).squeeze(-1)
        # Return probabilities (after sigmoid)
